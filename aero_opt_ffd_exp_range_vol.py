...
def objCon(funcs, printOK):
    ...
    # funcs["obj"] = (ap.V/9.81)*(thrust/mdot)*(funcs[ap["cl"]]/(funcs[ap["cd"]] + 0.04))*np.log(Wi/(Wi- 0.625*2*rhof*funcs['fuel_vol']))*(1/1000)  
    funcs["obj"] = (0.625*2*rhof*funcs['fuel_vol'])/32000 #funcs[ap["cd"]]
    
    # Assemble the objective and any additiona constraints:
    # funcs["obj"] = funcs[ap["cd"]]/1000 ## changed
    funcs["cl_con_" + ap.name] = funcs[ap["cl"]]  # cl calculation
    funcs["Range"+ap.name] = (ap.V/9.81)*(thrust/mdot)*(funcs[ap["cl"]]/(funcs[ap["cd"]] + 0.04))*np.log(Wi/(Wi- 0.625*2*rhof*funcs['fuel_vol']))*(1/1000) # range calculations
    if printOK:
...
# Add objective
optProb.addObj("obj", scale=1) #1e2
...
optProb.addCon("cl_con_" + ap.name, lower=0.5, upper=0.5, scale=2) # cl constraint
optProb.addCon("Range"+ap.name,lower=4400,scale=1/4400) ## adding the range constraint
